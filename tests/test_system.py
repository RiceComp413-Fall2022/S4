...
from worker.security import internal_required, api_required, INTERNAL_HEADERS, API_KEY

# #Perform Health Check
def test_health_check(start_server):
    """ Tests HealthCheck endpoint for all workers """
    worker_1 = requests.get("http://127.0.0.1:5001/HealthCheck", timeout=5, headers=INTERNAL_HEADERS)
    worker_2 = requests.get("http://127.0.0.1:5002/HealthCheck", timeout=5, headers=INTERNAL_HEADERS)
    worker_3 = requests.get("http://127.0.0.1:5003/HealthCheck", timeout=5, headers=INTERNAL_HEADERS)
...
def test_get_delete_object(start_server):
    ...
    get_res = requests.get("http://127.0.0.1:5002/GetObject", params={"key":"hello_output"}, headers={"X-Api-Key": API_KEY})
...
def test_list_objects(start_server):
    ...
    put_hello_world_obj()
    put_tetris_obj()
    list_res = requests.get("http://127.0.0.1:5001/ListObjects", headers={"X-Api-Key": API_KEY})
...
def put_tetris_obj():
    ...
    file.seek(0)
    worker_1 = requests.put("http://127.0.0.1:5001/PutObject", files={"file" : file}, params={"key":"OutputTetris"}, headers={"X-Api-Key": API_KEY})
    return worker_1
...
def put_hello_world_obj():
    ...
    file.seek(0)
    worker_1 = requests.put("http://127.0.0.1:5001/PutObject", files={"file" : file}, params={"key":"hello_output"}, headers={"X-Api-Key": API_KEY})
    return worker_1

def delete_hello_world_obj():
    """ Performs DeleteObject on a worker node for txt file"""
    worker_1 = requests.put("http://127.0.0.1:5002/DeleteObject", params={"key":"hello_output"}, headers={"X-Api-Key": API_KEY})
...
def delete_tetris_obj():
    ...
    worker_1 = requests.put("http://127.0.0.1:5002/DeleteObject", params={"key":"OutputTetris"}, headers={"X-Api-Key": API_KEY})
    print("Status_code for delete: " , worker_1.status_code)
    return worker_1
