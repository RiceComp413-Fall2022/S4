...
for i in range(len(instances)):
    ...
    if i < len(instances) - 1:
        node_url_to_instance_id[
            "http://" + instance.public_ip_address + f":{port}/"
        ] = instance.id
...
for i, node in enumerate(node_ips):
    ...
    if i < len(node_ips) - 1:  # worker nodes
        ...
        c.run(
            f'tmux new-session -d "cd S4 && source ./venv/bin/activate && cd main/src && flask run --host=0.0.0.0 -p {port}"',
            asynchronous=True,
        )
...
target_group = elb.create_target_group(
    Name="s4-nodes", Protocol="TCP", Port=port, TargetType="instance", VpcId=vpc_id
)
...
elb_dns = elb.describe_load_balancers(LoadBalancerArns=[balancer_arn])["LoadBalancers"][
    0
]["DNSName"]
wait_lb(elb_dns)
